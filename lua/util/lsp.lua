...
function M.common_on_init(client)
  local settings =
    string.format("%s/%s/settings.json", client.workspace_folders[1].name, moduleObject.settings.metadir)
...
function M.resolve_config(name, ...)
  ...
  local has_provider, cfg = pcall(require, "plugins.lsp.providers." .. name)
